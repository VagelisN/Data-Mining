...
'''
pipeline = Pipeline([ ('vect', CountVectorizer()),('tfidf', TfidfTransformer()),('clf', MultinomialNB(alpha = 0.001))])

params_svm = [{'clf__alpha':[4,3,2,1,0.5,0.1, 0.01, 0.001, 0.0001]}]
print "GridSearchCV for SVM with n_components = 100"
gs_svm=GridSearchCV(estimator=pipeline,param_grid=params_svm,scoring="accuracy",verbose=2,n_jobs=6,cv=10)
...
# Pipeline definition
pipeline = Pipeline([ ('vect', CountVectorizer()),('tfidf', TfidfTransformer()),('clf', MultinomialNB(alpha = 0.0001))
])
...
for train,test in folds.split(X, y):
    ...
    #i += 1
print "Finished 10-fold cross validation for MultinomialNB"
...
nb_scores=[total_accuracy/n_folds,total_precision/n_folds,total_recall/n_folds,total_f_score/n_folds]